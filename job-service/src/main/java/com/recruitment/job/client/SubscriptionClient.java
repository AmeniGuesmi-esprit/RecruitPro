...
public class SubscriptionClient {
    ...
    /**
     * Récupère l'abonnement ACTIF de la company (recruiterId) depuis subscription-service.
     * Retourne null si l'utilisateur n'a aucun abonnement actif OU si subscription-service
     * est injoignable (fail-closed : pas d'abonnement vérifiable = pas de création d'offre).
     */
    public SubscriptionInfo getActiveSubscription(Long companyId) {
        try {
            ResponseEntity<RemoteApiResponse<SubscriptionInfo>> resp = restTemplate.exchange(
                    "http://subscription-service/api/subscriptions/internal/active/" + companyId,
                    HttpMethod.GET,
                    null,
