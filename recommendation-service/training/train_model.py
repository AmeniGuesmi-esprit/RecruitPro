# -*- coding: utf-8 -*-
"""
Entraîne le modèle de RECOMMANDATION d'offres à partir d'un CV.
...
Pipeline :
  ...
  - Features numériques : taux de recouvrement des compétences, Jaccard,
    ratio d'expérience
  - GradientBoostingRegressor -> prédit un score de pertinence 0-100
...
def build_training_frame(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        feats["relevance_score"] = row["relevance_score"]
...
def main():
    ...
    parser.add_argument("--data", type=str, default="data/cv_job_recommendation_dataset.csv")
    ...
    X_train_raw, X_test_raw, y_train, y_test = train_test_split(
        feat_df, feat_df["relevance_score"], test_size=0.15, random_state=42
    )
    ...
    joblib.dump(model, models_dir / "recommend_model.joblib")
